...
import { acceptFamilyWithInvite, declineFamilyInvite, getFamilyInformation, getFamilyMembers } from '../../api/family';
...
const AcceptFamily = () => {
    ...
    const {data: familyMembers} = useQuery("familyMembers", () => getFamilyMembers(user.familyId))
    const {data: familyInformation} = useQuery("familyInformation", () => getFamilyInformation({familyId:user.familyId,userId: user.userId}))

    const acceptFamilyInviteSubmit =  (e) => {
        e.preventDefault()
        
        acceptFamilyWithInviteMutaiton.mutate({family_id: user.familyId, user_id: user.userId})
    ...
    const declineFamilyInviteSubmit = async (e) => {
        e.preventDefault()
        const response = declineFamilyInviteMutation.mutate({family_id: user.familyId, user_id: user.userId})
            ...
            <div className='accept-family-card'>
                <div className='accept-family-card-header'>
                    ...
                    <div className='accept-family-card-header-information'>
                        <p className='created-p'>{`created on ${familyInformation?.createDate.slice(0, 4)}`}</p>
                        <p className='family-name'>{`${familyInformation?.familyName} Family`}</p>
                        <p className='expire-p'>{`expires in ${familyInformation?.expiryDate} ${familyInformation?.expiryDate === 1 ? "day" : "days"}`}</p>
                    </div>
                </div>
